...
**Releases** are cut locally (CI does not build them): bump `versionCode`/`versionName` in `app/build.gradle.kts` (encode as `major*1000 + minor*10 + patch`, e.g. 1.2.0 → 1020; **hard bounds: patch ≤ 9, minor ≤ 99** — the formula only reserves one digit for patch, so 1.2.10 would collide with 1.3.0 at 1030 and Android refuses to "upgrade" to a non-increasing versionCode; when a line approaches the bound, bump minor/major instead or widen the encoding first) → add a `CHANGELOG.md` section → `./gradlew :app:assembleRelease` (output under `app/build/outputs/apk/release/`) → push a `vX.Y.Z` tag → `gh release create vX.Y.Z` with the APK attached. In-app update detection (`GitHubUpdateClient` + `UpdateRepository`) polls GitHub `releases/latest` and compares `tag_name` (minus the `v` prefix) against the installed `versionName`, so the release must be **published** (not draft/prerelease) to be picked up.
...
`CasLoginClient` performs this dance; `AuthRepository` orchestrates retry-from-saved-credentials in `tryRestoreSession()`. Saved credentials live in `SecureCredentialStore` (EncryptedSharedPreferences + Android Keystore master key)。**Keystore 故障分类（v1.11.1 起）**：加密 prefs 创建失败仅在**确定性损坏**（cause 链命中 `BadPaddingException`/AEADBadTag、或 Tink 的 `InvalidProtocolBufferException`——按类名匹配 shaded 包）或**连续 3 个进程**都失败时才走破坏性修复阶梯（删 prefs 文件 → 重置 master key）；其余（KeyStoreException/ProviderException 在真机上大量是开机早期/backend busy 的瞬时形态）不删任何数据、5s 退避后可重试。缓存策略是「成功才缓存」的 memoizer——**不要改回 `by lazy`**（会把首次瞬时失败的 null 钉死整个进程，连锁把 rememberMe 永久关掉）。`save()` 失败时 `AuthRepository.onLoginSuccess` 按 `isPermanentlyBroken()` 分叉：仅永久损坏才关 rememberMe，瞬时只发警告。`clear()` 在 prefs 拿不到时直接删加密文件（logout 语义优先，防已登出用户被残留密文复活）。`jwc.jxnu.edu.cn` is publicly reachable — **no campus network or VPN required**; do not surface such hints in error messages or help text.

**会话恢复阶梯（v1.5.0 起）**：任何静默重登都先走 `CasLoginClient.tryRefreshViaSso()`（不清 cookie、GET `casLoginEntry()`，本地 CAS TGC 仍有效时免密换新 jwc session，再以 `/User/Default.aspx` 解出学号确认），失败才退回完整账密 `login()`。`login()` 起手 `clearForHost(CAS_HOST)` 的语义保持不变（强制全新登录）。`AutoLoginThrottle` 指数退避护住**整条自动阶梯**——冷却窗口内连 SSO 续票也一并跳过（保守取舍：Transient 多为网络性失败，此时 SSO GET 同样会失败；冷却过期自愈）；真正提交密码、受 CAS 风控影响的只有账密那一步，SSO 续票另有本地 `hasCasTgc()` 前置检查，TGC 不在就直接跳过不发请求。
...
**jwc 无效会话的响应形态（v1.11.1 起防护，2026-07 实测）**：教务网不再把无会话请求 302 到 CAS/LoginAccount（v1.5.0 守卫建立在该旧行为上，形态一变即整体失效——「长时间闲置后课表不加载、个人信息丢失且永不自愈」的线上事故根因）。现行三形态：① `User/…` 系 URL → 302 落 `default.aspx?preurl=<原路径>`（200 + 74B「非法字符…参数错误」纯文本）；② `MyControl/All_Display.aspx` 系 → **零跳 200 直出**「【访问受限：请登录后访问！】【参数错误】」（60B）；③ `Public_Kkap` POST → 「Error:系统错误…」（51B，业务层 `CourseOfferingPage.isSystemError` 识别，**刻意不判 SessionExpired**——登录态下的真后端错误也返回同文案）。`JwcResponseGuard` 以两条正交判据拦 ①② 为 `SessionExpired`：**URL 判据**（落点 path 等于 `/default.aspx` 且 query 带 `preurl`，均大小写不敏感；放在 `validateJwcResponse` 层使 `getBytes` 图片路径同享）+ **短文本嗅探**（body ≤512 字符且同含「访问受限」「请登录」，`MAX_STUB_BYTES` 常量共享给 `JwcClient.getBytes` 的嗅探门——「像 HTML 或 ≤512B」都要过 `assertNotLoginPage`）。`probeSession()` 另有**语义判活**纵深防御：`readJwcHtml` 通过后还要 `UserDefaultPage.extractStudentId` 解得出学号才算 `Valid`（判活用降级正则、不要求中文姓名——留学生账号别误伤）；jwc 再换报错形态时冷启动最多退化为一次全量重登，不会再假 `Valid` 带占位 profile 进主界面。`SchedulePage.parse` 有 ddlSterm 锚点 fail-fast（缺失抛 `Decode`），防垃圾输入被静默解析成「合法空课表」写进缓存。改这些判据时同步 `JwcResponseGuardTest` / `UserDefaultPageTest` / `SchedulePageTest` 的对应用例。

`CasLoginClient.probeSession()` reuses `JwcResponseGuard`'s sniffing — a 200 that is actually a login page (or a redirect to CAS / SSO re-login / the `preurl` bounce) counts as **Invalid**, so a half-dead session is forced to re-auth instead of being let through. A let-through session silently degrades later fetches (e.g. the Profile name falling back to the placeholder 「同学」, college info stuck "加载中").
...
| 培养方案 (TrainingPlan) | `MyControl/All_Display.aspx?UserControl=xfz_bysh.ascx` (毕业审核页) | Yes | One scraper does both jobs: `GraduationAuditPage` extracts ① the「毕业最低学分」label/value (used by the Profile top stat card) and ② the full `TrainingPlan` (modules / required courses / optional sections / degree-course flags / minimum-credit thresholds), consumed by `TrainingPlanScreen`. Single network call serves both. |
...
- **品牌资产全矢量（v1.10.0 起）**：品牌标「开卷有成」（书页+对勾）只有一份母版几何，落在 5 个文件里（`drawable/ic_brand_logo|ic_launcher_foreground|ic_splash_logo` + `docs/assets/brand/logo{,-mark}.svg`）——改几何必须五处同步，参数与安全区换算见 `docs/brand.md`。仓库里**没有**图标 PNG（mipmap 只剩 anydpi 的 adaptive-icon XML）；登录页品牌标 tint 用 `onPrimary`（写死白色会在深色主题浅粉 primary 上消失）。
